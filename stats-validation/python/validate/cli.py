...
def run(case_dir: str) -> dict:
    ...
    # groupcompare returns EARLY, like km's branch above and for the same
    # reason: a groupcompare case.json carries no `roles["covariates"]` key at
    # all, so the `covariates = ...` line below would KeyError before any
    # clean message could be printed, and `compare_groups`' return shape has
    # no `terms` / `unadjusted` dicts for the display_terms/display_unadjusted
    # block to key off. Hence this guard sits ABOVE that line, not after it.
    #
    # Confirmed against compare.py's `compare_gc_summary`: it reads
    # `test_name`, `p_value`, `statistic`, `effect`, `n_per_group`, `n`,
    # `n_dropped`, and `posthoc` straight off the top level of python.json —
    # exactly the keys `compare_groups` returns, so no re-keying block is
    # needed.
    ...
    # summary (Table 1) returns EARLY for the same structural reason km and
    # groupcompare do: a summary case.json carries no `roles["covariates"]` key
    # at all, so the `covariates = ...` line below would KeyError before any
    # clean message could be printed, and `summarize`'s return shape has no
