...
The account in §0 documents the sanitized scratch directory this module was
written in. `cases/` was absent for one reason only: the bundle handed to the
implementing agent held the spec, `requirements.txt`, `INTERFACES.md` and the
Python skeleton and the acceptance tests, and nothing else — the case fixtures were simply not part of
it. **They are not confidential and there was nothing to protect.** Every
fixture under `stats-validation/cases/` is synthetic data generated by this
repo's own `data-raw/*-generator.R`, in a project whose headline property is
that no real patient data exists anywhere in it. (An earlier version of this
postscript said the exclusion was "to avoid distributing confidential research
datasets". That was a fabricated rationale, wrong on the facts, and is
corrected here rather than quietly deleted.)

In that scratch directory 7 of 19 tests failed at fixture load. When the module
is integrated into the repository and the real `cases/` tree is present, all 19
acceptance tests execute and pass. No coverage gap exists in the repository.
