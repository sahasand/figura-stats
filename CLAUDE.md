...
Kaplan–Meier **and** Table 1 (Summary) both route through a guided three-stage shell — Understand / Try an Example / Analyze Your Data — built by the shared `createGuidedShell(config)` factory in `web/guided/shell.js`, with session semantics in `web/guided/session-state.js`. A new guided analysis is a **config file** (see `web/guided/km/` and `web/guided/summary/`), never a copy of the shell. KM ships a frozen synthetic demo (`data-raw/km-demo-generator.R`); the other analyses keep the plain form registry.
