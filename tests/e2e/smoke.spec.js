const { test, expect } = require("@playwright/test");

// Boot check only: the guided specs cover full webR renders. This guards the
// nav — exactly the seven guided analyses, in DOM order, plus the empty state.
test("app boots with exactly the seven guided analyses", async ({ page }) => {
  await page.goto("/");
  const nav = page.locator("nav button[data-figure]");
  await expect(nav).toHaveCount(7);
  const labels = nav.locator(".nav-label");
  await expect(labels.nth(0)).toHaveText("Summary statistics");
  await expect(labels.nth(1)).toHaveText("Kaplan-Meier");
  await expect(labels.nth(2)).toHaveText("Group comparison");
  await expect(labels.nth(3)).toHaveText("Cox regression");
  await expect(labels.nth(4)).toHaveText("Logistic regression");
  await expect(labels.nth(5)).toHaveText("Linear regression");
  await expect(labels.nth(6)).toHaveText("Explore plot");
  // Not "on the left": under the 960px breakpoint the rail stacks ABOVE the
  // config pane, so the direction was wrong on every phone.
  await expect(page.getByText("Select an analysis to begin.")).toBeVisible();
});
